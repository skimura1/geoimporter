...
def upload_raster(geoserver, filepath, workspace):
    ...
    try:
        geoserver.create_coveragestore(
            layer_name=filename[:-4], path=filepath, workspace=workspace
        )
    except Exception:
...
def upload_postgis(filepath, engine):
    ...
    try:
        ...
        if not insp.has_table(filename[:-4], schema="public"):
            shp_file.to_postgis(filename[:-4], engine, index=True, index_label="index")
...
def upload_shapefile(geoserver, filepath, workspace, storename):
    ...
    try:
        geoserver.publish_featurestore(
            workspace=workspace, store_name=storename, pg_table=filename[:-4]
        )
